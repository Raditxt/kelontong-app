...
txs = db.execute(select(models.Transaction).order_by(models.Transaction.date.desc()).limit(200)).scalars().all()
return txs
...
db.add(models.TransactionItem(transaction_id=tx.id, product_id=prod.id, qty=item.qty, price=item.price))
